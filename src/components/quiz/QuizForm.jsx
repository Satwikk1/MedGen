...
const QuizForm = () => {
    ...
    function convertToObject(){
        ...
        for(let i=0; i<questionSet.length; i++){
            let questions = {}
            questions.question = questionSet[i][0];
            let optionArr = []
            for(let j=0; j<questionSet[i][1].length; j++){
                let option = {}
                option.txt = questionSet[i][1][j][0];
                option.status = questionSet[i][1][j][1];
                optionArr.push(option)
            }
            questions.options = optionArr;
                    ...
                    <div className="publish-quiz">
                        <button onClick={
                            ...
                            (e)=>{
                                ...
                                db.collection("Quiz").doc().set(object)
                                .then(() => {
                                    alert("Quiz Uploaded 👍");
                                })
                                .catch((error) => {
